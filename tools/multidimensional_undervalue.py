...
# Filter out rows with construction year above 2023
...
property_type = "House / Villa"  # Hardcoded for this example
...
if subset.empty:
    ...
    subset = subset.dropna(subset=['land_surface', 'price', 'construction_year'])
    ...
    if subset.empty:
        ...
        # Plot the relationship between land_surface and price, using construction year as the size of the points
        plt.figure(figsize=(12, 6))
        scatter = sns.scatterplot(data=subset, x='land_surface', y='price', hue='undervalued', size='construction_year', sizes=(20, 200),
                                  palette={True: 'red', False: 'blue'}, edgecolor='w', linewidth=0.5, alpha=0.6)
        ...
        plt.plot(subset['land_surface'], y_pred, color='green', linewidth=2, label='Linear Regression Line')
        plt.title(f'Land Surface vs. Price for {property_type} in {region} (Construction Year <= 2023)')
        ...
        print(f"Linear Regression Coefficients for {property_type} in {region} (Construction Year <= 2023):")
